...
**Ghi chú cập nhật khi triển khai trên nhánh `codex/macos-golden-gate-ui`:** Bản thử nghiệm chọn phương án tối thiểu để giảm rủi ro trong repo hiện tại. Lựa chọn phong cách được lưu ở khóa `ui_style` trong `app_settings.json` (có giá trị mặc định `classic` để tương thích cấu hình cũ), thay vì tạo file cấu hình thứ hai. `set_style()` dựng lại các widget trình bày nhưng không gọi `cancel_auto_next`; process, timer, log history, trạng thái can thiệp thủ công và các callback workflow được giữ lại. Đây là lựa chọn triển khai của nhánh này; phần kiến trúc registry/module tách riêng trong kế hoạch vẫn là hướng refactor tiếp theo nếu cần nhiều style hơn.
